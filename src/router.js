...
class Router {
  async handle(request, env, ctx) {
    ...
    if (path === '/web') return handleWeb(request, env, ctx);
    if (path === '/plst') return handlePlst(request, env, ctx);
    if (path === '/playlist') return handlePlaylist(request, env, ctx);
    if (path === '/seg') return handleSegment(request, env, ctx);
    return notFound();
  }
}
